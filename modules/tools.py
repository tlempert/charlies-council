...
def md_to_html(text):
    ...
    parts, pos = [], 0
    for m in _MD_CODE.finditer(cleaned):
...
_CITATION = re.compile(r" ?\[(\d+); (filing|calculation|media|search|judgment)\]")
_SOURCES_HEADING = "<h2>Sources and scope</h2>"


def memo_to_html(memo):
    """The memo's `[n; type]` tags as footnote marks that jump to source n in
    *Sources and scope*; the type rides along as the tooltip."""
    html = _CITATION.sub(r'<sup class="cite"><a href="#src-\1" title="\2">\1</a></sup>',
                         md_to_html(memo))
    head, heading, tail = html.partition(_SOURCES_HEADING)
    start, end = tail.find("<ol"), tail.find("</ol>")
    if heading and 0 <= start < end:
        numbered = itertools.count(1)
        items = re.sub(r"<li>", lambda _: f'<li id="src-{next(numbered)}">', tail[start:end])
        tail = tail[:start] + items + tail[end:]
    return head + heading + tail


PAGES_BASE = "https://tlempert.github.io/investor-reports"


def save_memo_html(ticker, memo, base_dir=None):
    """The investor memo as its own page: {TICKER}_Memo_{date}.html, one
    column, linking back to the dashboard. Returns {'memo_html': path} or {}."""
    if not memo or not str(memo).strip():
        return {}
    base_dir = base_dir or DEFAULT_REPORT_DIR
    os.makedirs(base_dir, exist_ok=True)
    m = re.search(r"^# (.+?)\s*$", clean_ansi(str(memo)), re.M)
    title = _html.escape(m.group(1)) if m else f"{ticker.upper()} — investor memo"
    date_display = datetime.now().strftime("%B %d, %Y")
    page = (
        '<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8">'
        '<meta name="viewport" content="width=device-width,initial-scale=1.0">'
        f'<title>{title}</title>'
        '<style>body{font-family:-apple-system,Georgia,serif;max-width:760px;margin:0 auto;padding:32px 20px;'
        'color:#1F2937;line-height:1.6;font-size:17px}h1{font-size:30px;line-height:1.2;margin-bottom:4px}'
        'h2{font-size:22px;margin-top:36px;border-bottom:1px solid #E5E7EB;padding-bottom:4px}h3{font-size:18px;margin-top:24px}'
        'table{border-collapse:collapse;margin:16px 0;width:100%;font-size:15px}th,td{border:1px solid #E5E7EB;padding:6px 10px;text-align:left}'
        'td:not(:first-child),th:not(:first-child){text-align:right}em{color:#4B5563}'
        '.meta{color:#6B7280;font-size:14px;margin-bottom:28px}.meta a{color:#2563EB}'
        'code{background:#F3F4F6;padding:1px 4px;border-radius:3px;font-size:14px}'
        'sup.cite{font-size:11px;line-height:0}sup.cite a{color:#2563EB;text-decoration:none}'
        'li:target{background:#FEF3C7}</style></head><body>'
        f'<div class="meta">Silicon Council investor memo · {ticker.upper()} · {date_display} · '
        f'<a href="{PAGES_BASE}/{ticker.upper()}.html">full council dashboard</a></div>'
        f'{memo_to_html(memo)}'
        f'<div class="meta" style="margin-top:40px">Generated by Silicon Council · {date_display}</div>'
        '</body></html>'
    )
    filename = f"{base_dir}/{ticker.upper()}_Memo_{datetime.now().strftime('%Y-%m-%d')}.html"
    try:
        with open(filename, "w", encoding="utf-8") as f:
            f.write(page)
        return {"memo_html": filename}
    except Exception as e:
        print(f"Error saving memo HTML: {e}")
        return {}
  ...
  <div class="accordion-body">
    ...
    if reports.get("memo"):
        memo_html = (f'<div class="tab-panel tab-content" id="tab-memo" style="display:none">'
                     f'<p class="subtitle"><a href="{PAGES_BASE}/{esc(ticker.upper())}_memo.html">Open as a standalone page ↗</a></p>'
                     f'{memo_to_html(reports["memo"])}</div>')
...
def deploy_report_to_github_pages(html_path: str, ticker: str) -> dict:
    ...
    return _push_pages(f"Deploy {ticker.upper()} report") or {"url": f"{PAGES_BASE}/{ticker.upper()}.html"}


def deploy_memo_to_github_pages(html_path: str, ticker: str) -> dict:
    """Publish the investor memo beside the dashboard as {TICKER}_memo.html.
    The index lists dashboards only; the dashboard's memo tab links here."""
    import shutil
    dest = f"{REPORTS_REPO}/{ticker.upper()}_memo.html"
    try:
        shutil.copy2(html_path, dest)
    except Exception as e:
        return {"error": f"Copy failed: {e}"}
    return _push_pages(f"Deploy {ticker.upper()} memo") or {"url": f"{PAGES_BASE}/{ticker.upper()}_memo.html"}


def _push_pages(message: str):
    """git add/commit/push in the reports repo. None on success, {'error'} on failure."""
    import subprocess
    try:
        subprocess.run(["git", "add", "."], cwd=REPORTS_REPO, check=True, capture_output=True)
        subprocess.run(["git", "commit", "-m", message], cwd=REPORTS_REPO, check=True, capture_output=True)
        subprocess.run(["git", "push", "origin", "main"], cwd=REPORTS_REPO, check=True, capture_output=True, timeout=30)
