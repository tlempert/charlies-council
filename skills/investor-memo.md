...
`## Valuation: the bet behind the price` — explain what a scenario table is before showing it. Table columns: `Five-year outcome | Weight | Terminal price | Annual return*`, one row per ledger scenario, weights as the ledger has them, with a footnote for the asterisk. Reconcile the weighted terminal prices to the central value and from there through the margin of safety to the ceiling and the floor, in one paragraph. Then `### The cross-check: what growth must occur?` — the ledger's `required_growth` rows as a table `Multiple | Required year-5 EPS | Required CAGR`, one sentence per row on whether the council's scenarios clear it, and a caveat paragraph on how precise these are (the ledger's `varied` ranges say so).
